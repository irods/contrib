...
\textbf{\huge iRODS Networking Performance}\\[20pt]
\textbf{\large Simulating A High-Bandwidth, High-Latency Use Case}\\[190pt]
...
defining of the window sizes had the effect of overriding the Linux kernel's TCP auto tuning\cite{autotuning} and severely reducing
overall throughput over higher latency connections.  Low latency connections were not affected.

In this whitepaper, iRODS 4.1.8 and 4.1.9 networking is compared across six variables: transfer command, file size, network RTT, TCP maximum buffer size, iRODS buffer size, and parallel threads.
...
\bibitem{slowstart} M. Allman, V. Paxson, E. Blanton (2009) {\em RFC5681: TCP Congestion Control} \url{https://tools.ietf.org/html/rfc5681}
