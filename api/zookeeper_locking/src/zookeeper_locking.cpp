...
int rs_zookeeper_lock( rsComm_t* _comm, dataObjInp_t* _inp ) {
...
extern "C" {
    ...
    irods::api_entry* plugin_factory(
        ...
        irods::apidef_t def = { DATA_OBJ_LOCK_AN,            // api number
                                ...
                                boost::any(std::function<
                                    int(rsComm_t*,dataObjInp_t*)>(
                                        rs_zookeeper_lock)), // the operation
								"data_obj_lock",             // operation name
