...
extern "C" {
                ...
                if ( ( result = ASSERT_PASS( ret, "Failed to get the cache name." ) ).ok() ) {
                    ...
                    if ( ( result = ASSERT_PASS( ret, "Failed to get the archive name." ) ).ok() ) {
                        ...
                        if ( ( result = ASSERT_PASS( ret, "Failed to get the resource name." ) ).ok() ) {
                            ...
                            rmKeyVal( &data_obj_inp.condInput, (char*)PURGE_CACHE_KW ); // do not want to accidentally purge
    ...
    irods::error compound_file_archive_object(
        irods::resource_plugin_context& _ctx ) {
        irods::error ret = _ctx.valid< irods::file_object >();
        if ( !ret.ok() ) {
            std::stringstream msg;
        ...
        return resc->call(
                   _ctx.comm(),
                   "resource_archive_object",
                   _ctx.fco() );

    } // compound_file_archive_object

    // delegate this operation directly to the archive object
    irods::error compound_file_restore_object(
        irods::resource_plugin_context& _ctx ) {
        irods::error ret = _ctx.valid< irods::file_object >();
        if ( !ret.ok() ) {
            std::stringstream msg;
        ...
        return resc->call(
                   _ctx.comm(),
                   "resource_restore_object",
                   _ctx.fco() );

    } // compound_file_restore_object
    ...
    irods::resource* plugin_factory( const std::string& _inst_name,
        ...
        resc->add_operation( "resource_archive_object",       "compound_file_archive_object" );
        resc->add_operation( "resource_restore_object",       "compound_file_restore_object" );
